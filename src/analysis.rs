...
use crate::ir::{ActionId, ArtifactId, EffectKind, Workflow};
...
pub struct Analysis {
    ...
    pub pure_actions: HashSet<ActionId>,
    ...
    /// as barriers.
    pub effect_barriers: HashSet<ActionId>,
}
...
impl Analysis {
    pub fn of(workflow: &Workflow) -> Analysis {
        ...
        for (idx, action) in workflow.actions.iter().enumerate() {
            ...
            } else if action.effects.iter().any(|&e| is_barrier(&workflow.effect(e).kind)) {
    ...
    pub fn is_pure(&self, action: ActionId) -> bool {
        self.pure_actions.contains(&action)
    }

    pub fn is_barrier(&self, action: ActionId) -> bool {
        self.effect_barriers.contains(&action)
    }
}

/// Privileged, externally-observable effects that pin ordering.
fn is_barrier(kind: &EffectKind) -> bool {
    matches!(kind,
        EffectKind::Deployment | EffectKind::PublishRelease
        | EffectKind::GitWrite | EffectKind::SecretAccess)
}
