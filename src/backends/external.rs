...
use super::{
    BackendKind, Capability, Catalogue, EmittingBackend, InstructionSummary,
    WorkflowCapabilities, plan_to_request,
};
...
impl EmittingBackend for ExternalBackend {
    ...
    fn backend_kind(&self) -> BackendKind { BackendKind::Custom(self.id.clone()) }
    fn capabilities(&self) -> Vec<Capability> { vec![] }
    fn workflow_capabilities(&self) -> WorkflowCapabilities { self.workflow_caps }
    ...
    fn emit(&self, plan: &Plan) -> Result<String, Vec<Diagnostic>> {
        ...
        let plan_json = serde_json::to_value(&req).map_err(|e| vec![
            Diagnostic::error(crate::diagnostics::DiagCode::UnknownAction, format!("serialize plan: {e}"))
        ])?;
        let response = call(&self.executable, json!({"op": "emit", "plan": plan_json}))
            .map_err(|e| vec![
                Diagnostic::error(crate::diagnostics::DiagCode::UnknownAction, format!("external backend: {e}"))
            ])?;
        response["output"].as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| vec![
                Diagnostic::error(crate::diagnostics::DiagCode::UnknownAction, "external backend: missing 'output' field in response")
            ])
    }
...
fn call(executable: &PathBuf, request: JsonValue) -> Result<JsonValue, String> {
    ...
    child.stdin.take().unwrap().write_all(request_line.as_bytes())
