...
impl Backend for GithubActionsBackend {
    ...
    fn workflow_capabilities(&self) -> crate::backends::WorkflowCapabilities {
        ...
        WC::JOBS | WC::DEPENDENCIES | WC::CONDITIONS | WC::MATRICES
            | WC::PERMISSIONS | WC::SECRETS | WC::APPROVALS | WC::RUNNER_SELECTION
    }
    ...
    match kind {
        "github.uses" => {
            ...
            let (step_name, with) = match op {
                LogicalOp::UploadArtifact { name, path, lifetime } => {
        ...
        "github.run.native" => {
            let LogicalOp::Native { id, fallback, .. } = op else { return vec![] };
            vec![GhStep {
                name: format!("Run {id}"),
        ...
        "github.run" => {
            let LogicalOp::RunShell { label, script, env } = op else { return vec![] };
        ...
        "github.environment" => {
            ...
            let LogicalOp::RequestApproval { reason } = op else { return vec![] };
            vec![GhStep {
    ...
    if triggers.is_empty() {
        return GhOn {
            push: if opts.push_branches.is_empty() {
                ...
                Some(GhPush { branches: opts.push_branches.clone(), tags: vec![] })
            },
            pull_request: if opts.pull_request { Some(EmptyMapping) } else { None },
            schedule: None,
...
fn concurrency_of(c: &crate::ir::Coordination) -> GhConcurrency {
    GhConcurrency { group: c.group.clone(), cancel_in_progress: c.cancel_in_progress }
}
...
mod tests {
    ...
    fn pull_request_trigger_serializes_as_empty_mapping() {
        assert_eq!(YamlRenderer::<EmptyMapping>::default().render(&EmptyMapping).trim(), "{}");
    }
    ...
    fn triggers_drive_the_on_block() {
        ...
        b.trigger(Trigger::Push { branches: vec!["main".into()] });
        b.trigger(Trigger::Tag { pattern: "v*".into() });
        b.trigger(Trigger::Schedule { cron: "0 2 * * *".into() });
        ...
        assert!(!yaml.contains("pull_request:"), "no PR trigger declared: {yaml}");
    }
    ...
    fn timeout_and_coordination_emit() {
        ...
        wf.action_calls[a.idx()].coordination =
    ...
    fn artifact_lifetime_sets_retention_days() {
        ...
        let yaml = emit(&plan(&wf).unwrap(), &EmitOptions::default());
        assert!(yaml.contains("retention-days: '14'") || yaml.contains("retention-days: 14"), "{yaml}");
    }
    ...
    fn native_publish_upgrades_to_uses_when_inventory_permits() {
        ...
        let yaml = emit(&plan(&wf).unwrap(), &EmitOptions::default());
        assert!(yaml.contains("uses: pypa/gh-action-pypi-publish@release/v1"), "{yaml}");
    ...
    fn render_upload_includes_with_fields() {
        ...
        let sel = Selector::for_backend(&backend);
        let caps = instructions::capabilities();
        let op = LogicalOp::UploadArtifact { name: "my-binary".into(), path: Some("dist/app".into()), lifetime: None };
