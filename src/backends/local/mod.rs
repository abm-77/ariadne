...
pub use runtime::{
    ContainerRuntime, ContainerSpec, DockerRuntime, Mount, PodmanRuntime, RunResult, RuntimeError,
};
...
impl LocalBackend<PodmanRuntime> {
    pub fn podman() -> Self {
        Self::new(PodmanRuntime::default())
    }
}
        ...
        let units = plan
            ...
            .map(|unit| {
                let lines: Vec<String> = unit
                    ...
                    .collect();
                BashUnit {
                    label: unit.action_name.to_string(),
                    lines,
                }
            })
            .collect();
...
mod tests {
    ...
    fn backend_trait_reports_kind_and_capabilities() {
        ...
        assert!(
            backend
                .capabilities()
                .contains(&Capability::new("process.exec"))
        );
    }
    ...
    fn approval_gate_emits_read_prompt_and_guard() {
        ...
        let sel = Selector::for_backend(&backend);
        let caps = instructions::capabilities();
        let op = LogicalOp::RequestApproval {
            reason: "deploy to prod".into(),
        };
        let selected = sel.select(&op, &caps, &[]).unwrap();
        let lines = lower_op(&op, selected.instruction);
