use super::{arg_str, def, local, LoweringBody, Registry};
...
pub fn register(r: &mut Registry) {
    ...
    r.register(def("package.install.pip", "package.install", "pip", |a| {
        local(vec!["pip".into(), "install".into(), arg_str(a, "package").unwrap_or_default()])
    }));
    r.register(def("package.install.cargo", "package.install", "cargo", |a| {
        local(vec!["cargo".into(), "install".into(), arg_str(a, "package").unwrap_or_default()])
    }));
    r.register(def("package.install.npm", "package.install", "npm", |a| {
        local(vec!["npm".into(), "install".into(), "-g".into(), arg_str(a, "package").unwrap_or_default()])
    }));
    r.register(def("package.install.apt", "package.install", "apt", |a| {
        local(vec!["sudo".into(), "apt-get".into(), "install".into(), "-y".into(), arg_str(a, "package").unwrap_or_default()])
    }));
    r.register(def("package.install.dnf", "package.install", "dnf", |a| {
        local(vec!["sudo".into(), "dnf".into(), "install".into(), "-y".into(), arg_str(a, "package").unwrap_or_default()])
    }));
}
