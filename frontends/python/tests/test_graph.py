...
class TestSimpleWorkflow:
    ...
    def test_actor_declared(self):
        ...
        tir = json.loads(ci().emit_json())
        assert tir["inventory"]["actors"][0]["id"] == "github-ubuntu"
        assert "ubuntu-latest" in tir["inventory"]["actors"][0]["labels"]
    ...
    def test_topological_order(self):
        ...
        tir = json.loads(ci().emit_json())
        names = [a["name"] for a in tir["action_calls"]]
...
class TestHandleSemantics:
    ...
    def test_handle_passed_as_input(self):
        ...
        tir = json.loads(ci().emit_json())
        checkout_out = tir["action_calls"][0]["outputs"][0]
        build_in = tir["action_calls"][1]["inputs"][0]
    ...
    def test_multiple_consumers_of_same_artifact(self):
        ...
        tir = json.loads(ci().emit_json())
        wheel_id = tir["action_calls"][1]["outputs"][0]

        consumers = [a for a in tir["action_calls"] if wheel_id in a.get("inputs", [])]
    ...
    def test_rebinding_creates_new_artifact(self):
        ...
        tir = json.loads(ci().emit_json())
        assert len(tir["action_calls"]) == 4

        build_out = tir["action_calls"][1]["outputs"][0]
        repair_in = tir["action_calls"][2]["inputs"][0]
        repair_out = tir["action_calls"][2]["outputs"][0]
        check_in = tir["action_calls"][3]["inputs"][0]
...
class TestExternalArtifact:
    ...
    def test_external_artifact_used_as_input(self):
        ...
        assert src_id in tir["action_calls"][0].get("inputs", [])
...
class TestScalarParameters:
    def test_scalar_param_used_in_impl(self):
        ...
        tir = json.loads(ci().emit_json())
        bw = next(a for a in tir["action_calls"] if a["name"] == "versioned_wheel")
...
class TestConsequences:
    def test_named_consequence_in_tir(self):
        ...
        tir = json.loads(ci().emit_json())
        assert any(e["name"] == "deploy" for e in tir.get("consequences", []))
        deploy_csq = next(e for e in tir["consequences"] if e["name"] == "deploy")
        assert deploy_csq["kind"] == "Deployment"
        assert deploy_csq["requires_approval"] is True

    def test_auto_named_consequence(self):
        @action(outputs={}, consequences=[ConsequenceKind.Network])
        def fetch_data():
        ...
        tir = json.loads(ci().emit_json())
        assert any(e["name"] == "fetch_data.network" for e in tir.get("consequences", []))

    def test_consequence_id_referenced_in_action(self):
        @action(
            outputs={},
            consequences=[Consequence("release", ConsequenceKind.PublishRelease)],
        )
        def publish():
        ...
        tir = json.loads(ci().emit_json())
        consequence_ids = tir["action_calls"][0].get("consequences", [])
        assert len(consequence_ids) == 1
        assert tir["consequences"][consequence_ids[0]]["name"] == "release"


class TestSecrets:
    def test_secrets_in_action(self):
        @action(outputs={}, secrets=["DEPLOY_TOKEN"])
        def deploy():
        ...
        tir = json.loads(ci().emit_json())
        assert "DEPLOY_TOKEN" in tir["action_calls"][0].get("secrets", [])


class TestConstraints:
    def test_label_constraint_in_action(self):
        @action(outputs={"image": SourceTree}, constraints=[Constraint.label("gpu")])
        ...
        tir = json.loads(ci().emit_json())
        act = next(a for a in tir["action_calls"] if a["name"] == "train")
...
class TestContextManager:
    def test_graph_as_context_manager(self):
        ...
        assert len(tir["action_calls"]) == 2

    def test_context_not_active_outside(self):
        from ariadne._graph import current_graph
...
class TestMultiOutput:
    def test_multi_output_action(self):
        @action(outputs={"binary": Binary, "report": TestReport})
        ...
        tir = json.loads(ci().emit_json())
        bat = next(a for a in tir["action_calls"] if a["name"] == "build_and_test")
