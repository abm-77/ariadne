...
pipeline is made of (artifacts, actions, consequences, constraints, policies)
and Ariadne figures out *how* to run it: it validates the workflow, builds a
correct execution plan, optimizes artifact movement and runner placement, and
emits backend-specific CI configuration such as GitHub Actions YAML.
...
(cargo, maturin, pytest, git, and so on); Ariadne selects one and lowers it.
...
matrices, artifact upload/download steps, caches, runner labels. This is tedious
...
Author intent  →  Thread IR  →  Ariadne  →  Execution plan  →  Backend config
                                (validate, analyze, plan, optimize)
```

The guarantee at the center of the design is that **Ariadne always produces a
correct plan if one exists.** Optimization is optional and never required for
...
| **Consequence** | External mutation or privileged interaction (Deployment, PublishRelease, SecretAccess, GitWrite, Network, CommentOnPr) |
| **Placement** | How an artifact is physically realized or accessed (artifact store, shared volume, cache, registry, local path) |
| **Actor** | An execution resource (github-ubuntu, self-hosted-gpu, local-docker) |

These are deliberately not collapsed into generic jobs and steps. An action
names a **semantic operation** such as `build.binary`, not a command. A raw
`shell(...)` escape hatch exists for tool-specific work, with typed boundaries
the compiler trusts (declared inputs, outputs, consequences, secrets).

## From intent to a step: two phases

Every operation in a plan has one uniform identity: a `dialect.action` (the
*logical op*). **Specification** binds it to an implementation, producing a
*specified op* `dialect.action.impl`. **Instruction selection** then lowers that
to a backend step.

```
Logical op        dialect.action          e.g. build.binary, scm.checkout, ci.artifact.upload
    ↓   specification           (plan-time, backend-agnostic)
...
**Specification** (`src/specifications/`) picks the implementation from the inventory.
The same `test.unit` becomes `test.unit.cargo` under `use("cargo")` or
`test.unit.pytest` under `use("pytest")`.
...
`uses: actions/checkout@v4` on GitHub or `git checkout .` elsewhere, and
...
`ci.artifact.download`, `ci.artifact.transfer`, `ci.cache.restore`,
`ci.cache.save`, `ci.approval`.

## How it works

The engine is a phase pipeline. Each phase is a module, and data flows inward to
outward:

```
ir → diagnostics → validate → analysis → specification → planner → optimize → backends
```

1. **validate**: semantic checks, type and consequence safety, structured
   diagnostics.
2. **analysis**: purity, consumer counts, consequence barriers (the safety
   substrate for reordering).
3. **specification**: choose an implementation per semantic action from
   the inventory (backend-agnostic).
...
5. **optimize**: a stack of passes, gated by compiler-style `-O` levels, each
   ...
   - `placement`: copy becomes mount when the backend and actors support it
   ...
   - `dedup`, `fusion`, `sibling_fusion`: eliminate redundant work and pack
     independent same-runner jobs (consequence-aware, cost-arbitrated)
6. **backends**: instruction selection and emission (GitHub Actions YAML, or a
...
# Why this plan? Show planner and optimization decisions, the specified op for
# each step (e.g. "build.binary.cargo: cargo build ..."), and instruction
# selection when you name a backend.
loom explain tests/fixtures/simple-build-test.tir.json github

# Does it behave correctly? Run the workflow's test suite, or execute once in
# Podman if no suite is configured.
...
| `-O1` | placement and actor optimization |
| `-O2` | adds colocation and parallelization *(default)* |
| `-O3` | adds deduplication and fusion (vertical and sibling) |
